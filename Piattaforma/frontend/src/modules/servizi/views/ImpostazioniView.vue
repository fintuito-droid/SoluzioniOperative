          ...
          <v-card-text>
            ...
            >
              ...
              <template #item.moduli="{ item }">
                <v-chip v-for="m in item.moduli" :key="m" size="x-small"
      ...
      <v-card>
        ...
        <v-card-text class="pt-4">
          <v-row dense>
            ...
            <v-col cols="12">
              <div class="text-caption text-medium-emphasis mb-1">Moduli abilitati</div>
              <v-chip v-if="formUtente.ruolo === 'admin'" size="small" color="primary" variant="tonal">
                Gli amministratori accedono a tutti i moduli
              </v-chip>
              <template v-else>
                <v-checkbox v-for="m in MODULI" :key="m.codice"
                            v-model="formUtente.moduli" :value="m.codice"
                            :label="m.nome" density="compact" hide-details/>
              </template>
            </v-col>
            <v-col cols="12">
...
const hUtenti = [
  ...
  { title: 'Moduli',         key: 'moduli',         sortable: false },
...
function nomeModulo(codice) {
  return MODULI.find(m => m.codice === codice)?.nome || codice
}

const formUtente = reactive({
  id: null, username: '', password: '', ruolo: 'dipendente',
  personale_id: null, comando_id: null, attivo: true, moduli: ['servizi'],
})
