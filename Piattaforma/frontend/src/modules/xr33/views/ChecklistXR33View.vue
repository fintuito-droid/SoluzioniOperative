            ...
            <v-form @submit.prevent="salvaChecklist">
              <v-row>
                <v-col cols="12" md="6" lg="3">
                  <v-text-field
                    :model-value="comandoCorrente"
                    label="Comando"
                    variant="outlined"
                    prepend-inner-icon="mdi-office-building"
                    readonly
                    :loading="loadingComando"
                  />
                </v-col>
                ...
                <v-card-text>
                  <v-row>
                    <v-col cols="12" md="4" lg="3">
                      ...
                      >
                        <v-tab
                          v-for="gruppo in gruppiControlli"
                          :key="gruppo.key"
                          :value="gruppo.key"
                          class="justify-start"
                        >
                          <v-icon start>{{ gruppo.icon }}</v-icon>
                          <span class="me-2">{{ gruppo.label }}</span>

                          <v-spacer />

                          <v-chip
                            v-if="contaKO(gruppo) > 0"
                            color="red-darken-1"
                            size="small"
                            variant="flat"
                          >
                          ...
                          <v-chip
                            v-else-if="contaNonCompilati(gruppo) > 0"
                            color="blue-grey-lighten-1"
                            size="small"
                            variant="flat"
                          >
                            {{ contaNonCompilati(gruppo) }} NC
                          </v-chip>
                    ...
                    <v-col cols="12" md="8" lg="9">
                      <v-window v-model="tabAttivo">
                        ...
                        >
                          <v-row>
                            <v-col
                              v-for="controllo in gruppo.controlli"
                              :key="controllo.key"
                              cols="12"
                              lg="6"
                            >
                              <v-card
                                variant="outlined"
                                class="pa-3 h-100 controllo-card"
                                :class="classeControllo(controllo.key)"
                              >
                                ...
                                <v-btn-toggle
                                  :model-value="form.controlli[controllo.key].stato"
                                  mandatory
                                  divided
                                  class="mb-3"
                                  @update:model-value="setStatoControllo(controllo.key, $event)"
                                >
                                  <v-btn value="NC" size="small">
                                    Non compilato
                                  </v-btn>
                                  <v-btn value="OK" size="small">
                                    OK
                                  </v-btn>
                                  <v-btn value="KO" size="small">
                                    KO
                                  </v-btn>
                                </v-btn-toggle>

                                <v-chip
                                  :color="coloreChipStato(form.controlli[controllo.key].stato)"
                                  variant="flat"
                                  size="small"
                                  class="mb-3"
                                >
                                  {{ labelStato(form.controlli[controllo.key].stato) }}
                                </v-chip>
...
const controlliConfig = gruppiControlli.flatMap((g) => g.controlli);
...
function resetForm() {
  ...
  form.operatori = [];
  form.note = "";
  tabAttivo.value = "alimentazione";

  for (const controllo of controlliConfig) {
    form.controlli[controllo.key].stato = "NC";
...
function setStatoControllo(key, valore) {
  form.controlli[key].stato = valore;
}

function labelStato(stato) {
  if (stato === "OK") return "OK";
  if (stato === "KO") return "KO";
  return "Non compilato";
}

function coloreChipStato(stato) {
  if (stato === "OK") return "green-accent-4";
  if (stato === "KO") return "red-darken-1";
  return "blue-grey-lighten-1";
}

function classeControllo(key) {
  const stato = form.controlli[key].stato;

  if (stato === "OK") return "controllo-card controllo-card-ok";
  if (stato === "KO") return "controllo-card controllo-card-ko";
  return "controllo-card controllo-card-nc";
}

function contaKO(gruppo) {
  return gruppo.controlli.filter(
    (controllo) => form.controlli[controllo.key].stato === "KO"
  ).length;
}

function contaNonCompilati(gruppo) {
  return gruppo.controlli.filter(
    (controllo) => form.controlli[controllo.key].stato === "NC"
  ).length;
}

function statoToEsito(stato) {
  if (stato === "OK") return "OK";
  if (stato === "KO") return "KO";
  return null;
}

async function caricaComando() {
  loadingComando.value = true;

  try {
    const token = getToken();

    const response = await fetch(`${API_BASE}/me/comando`, {
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Errore caricamento comando: ${testoErrore}`);
    }

    const data = await response.json();
    comandoCorrente.value = data.nome ?? data.descrizione ?? data.sigla ?? "";
  } catch (err) {
    console.error(err);
    errore.value = err.message || "Errore nel caricamento del comando.";
  } finally {
    loadingComando.value = false;
  }
}
...
async function salvaChecklist() {
  ...
  try {
    ...
    for (const controllo of controlliConfig) {
      const stato = form.controlli[controllo.key].stato;
...
.controllo-card-ok {
  ...
  box-shadow: 0 0 0 1px rgba(57, 211, 83, 0.16);
}

.controllo-card-ko {
  background-color: #fff4f4;
  border: 2px solid #ef5350;
  box-shadow: 0 0 0 1px rgba(239, 83, 80, 0.12);
}

.controllo-card:hover {
